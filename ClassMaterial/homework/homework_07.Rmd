...
`lm(variable2 ~ variable1, data = df)` rather than `lm(df$variable2 ~
df$variable1)`. This is important for getting `predict()` to return only a
single value when you're using the model to predict a single point, and when
using `lm()` to do multiple regression.
