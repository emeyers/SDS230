...
To do this, use a for loop where the index of the for loop where the counter
index `i` goes from 1 to 5. Then, inside the for loop do the following:
...
The highest $R^2$ value occurs with the degree 5 polynomial. However, since
$R^2$ only increase with more variables, this is expected. We will need to use
some of the techniques we will discuss next week to really decide which model to
use (and plotting the models below will give us some insight too).
...
Run three-fold cross-validation where you:
...
Please reflect on how the homework went by going to Canvas, going to the Quizzes link, and clicking on [Reflection on homework 8](https://yale.instructure.com/courses/79947/quizzes/52431).
