...
through Gradescope by 11pm on Sunday December 4th.
...
In this homework you will analyze data from a cognitive science experiment on how
the human visual system processes information. In particular, it is well known
that when an object is very visually distinct from its surroundings, the object 
appears to "pop-out" and humans can rapidly identify the object. However, it is
not known whether reaction times are affected by the position of where the object 
appears, or if having multiple irrelevant distractor objects affects reaction 
times. Understanding humans process different visual stimuli could lead to better
object recognition systems and could give insight into neural disorders (e.g.,
simultanagnosia).
...
If you are interested in trying the experiment to better understand it, an
online version of the experiment can be found at:
https://run.pavlovia.org/xfang/popout_exp1/html/  If you enter your email
address in the "participant" name field, we can also give you a copy of your
data.
...
variable is called `mean_rt` and should have the mean reaction time for each
group (position). The second variable is called `rt_residuals` and should
contain the difference between the reaction time for each trial and the mean
reaction time for the corresponding position (i.e., the values in the `mean_rt`
variable). Using the `group_by()` function in conjunction with the `mutate()`
function will be helpful for adding these variables to the data frame.
...
To do this, recreate the `popout_residuals` data from the `popout_log_data` data 
frame, and then use the recreated `popout_residuals` to fill in the ANOVA table, 
which will be easier than using the `popout_log_data` data frame. Be sure to
print out all the values as you compute them in the R chunk below to "show
your work" (i.e., print out the dfs, SSG, SSE, MSG, etc.). You will fill in
the p-value in this table later during part 1.5 of this homework.
...
**Exercise 1.3 (5 points)**: Now it's time to do step 3 of the hypothesis test
by plotting the null distribution. To create the appropriate F-distribution, use
the degrees of freedom you calculated in part 1.2e. Then use either base R
graphics or ggplot to plot the F-distribution density function, and add the
observed statistic as a red vertical line to the plot. From looking at this null
...
**Part 2.1 (9 points)**:  Start your hypothesis test with step 1 by stating the
...
[Reflection on homework 10](https://yale.instructure.com/courses/79947/quizzes/52427).
