...
For example, if $e^{\hat{\beta}_1} = 0.1$, then if $\hat{f}(x) = 100$,
$\hat{f}(x + 1) = 1000$, and if $\hat{f}(x) = 1,000,00$, then $\hat{f}(x + 1) =
10,000,000$
...
Using the $R^2$ statitic, do men or women salaries better predict a school's
endowment?
