...
# plot the data after taking a log transformation of the reponse variable y, which is a school's endowment
...
For example, if $e^{\hat{\beta}_1} = 0.1$, then if $\hat{f}(x) = 100$, $\hat{f}(x + 1) = 1000$, and if $\hat{f}(x) = 1,000,00$, then $\hat{f}(x + 1) = 10,000,000$
...
When building multiple regression models, if certain predictors are linear combinations of other predictors (i.e., roughly speaking, this means that predictors are strongly correlated) then issues can arrise that affect our ability to infer the regression coefficients for these predictor. Let's explore this now...
...
Using the $R^2$ statitic, do men or women salaries better predict a school's endowment? 
