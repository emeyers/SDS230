...
IPED_4 <- IPED_3 |>
  mutate(assist_ind = ifelse(rank_name == "Assistant", 1, 0)) |>
  mutate(assoc_ind = ifelse(rank_name == "Associate", 1, 0))
...
lm_manual_offset <- lm(salary_tot ~ log_endowment + assoc_ind + assist_ind, data = IPED_4)

summary(lm_manual_offset)
...
lm_auto_offset <- lm(salary_tot ~ log_endowment + rank_name, data = IPED_4)

summary(lm_auto_offset)
...
IPED_5 <- IPED_4 |>
  mutate(assist_endow_interaction = assist_ind * log_endowment) |>
  mutate(assoc_endow_interaction = assoc_ind * log_endowment)
...
lm_manual_interaction <- lm(salary_tot ~ log_endowment + assoc_ind + assist_ind + assoc_endow_interaction + assist_endow_interaction, 
                            data = IPED_5)

summary(lm_manual_interaction)



# Fit model with automatically added indicator vectors

lm_auto_interaction <- lm(salary_tot ~ log_endowment * rank_name, data = IPED_5)

summary(lm_auto_interaction)
...
anova(lm_auto_offset, lm_auto_interaction)
...
lm_fit <- lm(log(endowment) ~ salary_tot, data = assistant_data)
...
lm_men <- lm(log(endowment) ~ salary_men, data = assistant_data)
summary(lm_men)

# fit a model using only women's salary
lm_women <- lm(log(endowment) ~ salary_women, data = assistant_data)
summary(lm_women)
