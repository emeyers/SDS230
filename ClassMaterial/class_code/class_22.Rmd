...
To start our mapping practice, let's plot Yale's location (longitude = -72.9223,
latitude = 41.3163) on a map of Connecticut.
...
To get a sense of which states have an older population, let's plot the number
of people in each state who are over 64 years old.
...
In order to really see which states have older populations, we need to normalize
our maps based on the total population.
...
If we want to create a side-by-side box plot in ggplot we would need a data
...
Let's transform the data to make it longer so that it has these columns using
the `pivot_longer(df, cols)` function.
