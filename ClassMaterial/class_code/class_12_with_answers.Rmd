...
mean(sample_heights) + z_star * SE_boot * c(-1, 1)


# 99% CI using the percentile method
(CI_boot_percentile <- quantile(bootstrap_dist, c(.005, .995)))
...
flights_small <- sample_n(flights, 25)

# number of rows in the flights data frame
nrow(flights_small)

# number of rows in the weather data frame
nrow(weather)
...
joined_flights_weather <- left_join(flights_small, weather)

nrow(joined_flights_weather)
...
joined_flights_weather <- left_join(flights_small, weather, by = "day")
