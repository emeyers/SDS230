...
SDS230::download_image("pipe.jpg")


# SDS230::download_data("movies.Rdata")

# problem download file on Windows so using this to download data as binary data
download.file("https://raw.githubusercontent.com/emeyers/SDS230/master/ClassMaterial/data/movies.Rdata", "movies.Rdata", mode = "wb")
...
# create a function called cube_it() that takes a value to the third power
...
# create the `power_it()` function that takes a value to a particular power
...
# a function that uses an if statement
...
To explore the dplyr functions, we will use data that consists of random sample of movies that were scraped from the [Rotten Tomatoes](https://www.rottentomatoes.com/) and [IMDB](https://www.imdb.com/) websites. A codebook describing the variables in this dataframe and be found on [this website](http://www2.stat.duke.edu/~mc301/data/movies.html).
...
# use the dim() function to get the number of rows and columns in the data frame
...
# what are the title unique in the title_type variable?
...
# let's look at the dimensions of the data frame and the first few rows using the head() function
...
#### Part 1.6: Summarizing data summarize()

We can summarize data in particular columns (i.e. particular variables) using the summarize() function. 
...
# get the mean critics_score and audience_score
...
We can group data by the values in a particular (usually categorical) variable using the group_by() method. This method does nothing on it's own, but is useful in conjunction with other dplyr functions, particularlly the summarize function. 
...
# get the mean critics_score and audience_score separately for each movie genre
...
# let's pipe in the first argument to the filter() function 
...
Please explore the data more using the dplyr functions to find an additional interesting trend and report what you find in the week 5 class survey. We can discuss interesting findings in class on Thursday. 

Also, remember to fill out the full 5 class survey can be found at: https://yale.instructure.com/courses/61201/quizzes/31863
