...
Start on section 4 (sections 1-3 were done in class 17)
...
When making inferences about regression coefficients, there are a number of assumptions that need to be met to make these tests/confidence invervals valid. The assumptions are: 
...
**Part 4.1:** To check whether the residuals are normally distributed we can use create a Q-Q plot. The `car` package has a nice function to create these plots called qqPlot() to create these plots that uses the residuals from model fit with the lm() function, i.e, the `lm_fit` object.
...
**Part 4.2:** To check for homoscedasticity and linearity, we can create a plot of the residuals as a function of the fitted values. 
...
**Part 4.3:** To check if the data points are independent requires knowledge of how the data was collected. Do you think the data is independent here? i.e., do you think that the salary of one school effects the salaries at another school? 
...
# mutate on the hat-values and plot them in color
assistant_data %>%
...
**Part 5.2:** Let's explore the standardized and studentized residuals using the `rstandard()` and `rstudent()` functions. Are any of the standardized or studentized residuals greater than 3? 
